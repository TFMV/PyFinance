...
def main(ticker):
    ...
    stock_data['EMA'] = calculate_ema(stock_data['Close'], span=10)
    ...
    scaler_stock_prices = MinMaxScaler(feature_range=(0, 1))
    scaler_ema_prices = MinMaxScaler(feature_range=(0, 1))
    scaler_fft_features = MinMaxScaler(feature_range=(0, 1))

    scaled_stock_prices = scaler_stock_prices.fit_transform(stock_prices)
    ...
    # Combine stock prices, EMA, and FFT features as inputs
    scaled_features = np.hstack((scaled_stock_prices, scaled_ema_prices, scaled_fft_features))

    def create_dataset(data, time_step=1):
        X, Y = [], []
        for i in range(len(data) - time_step - 1):
            X.append(data[i:(i + time_step), :])
            Y.append(data[i + time_step, 0])  # Predicting the stock price
    ...
    predictions_stock = scaler_stock_prices.inverse_transform(predictions.reshape(-1, 1))
    y_test_stock = scaler_stock_prices.inverse_transform(y_test.reshape(-1, 1))

    # Calculate MAE
    mae = mean_absolute_error(y_test_stock, predictions_stock)

    # Print results
    print("Predicted Prices:", predictions_stock.flatten())
    print("Actual Prices:", y_test_stock.flatten())
